(function (global, factory) {
    ...
    (ConvexHull.prototype.prep = function () {
        ...
        if (v3 === null) {
            throw new error('Not enough non-planar Points (v3 is null)');
            return;
        }
    ...
    function polygonClip(clip, subject, siteObj) {
                ...
                for (var e = 0; e < 3; e++) {
                    ...
                    if (!verticesVisited[destVertex.index]) {
                        ...
                        if (!site.isDummy && d3Polygon.polygonLength(site.nonClippedPolygon) > 0) {
                            ...
                            site.hull = d3Polygon.polygonHull(clippedPoly);
                            if (site.nonClippedPolygon.length === 0) {
                                console.log("errpr")
                            }
                            ...
                            site.centroid = d3Polygon.polygonCentroid(site.hull);
    ...
    function weightedVoronoi() {
        ...
        function _weightedVoronoi(data) {
            ...
            let polygons = [];
            try {
                polygons = computePowerDiagramIntegrated(formatedSites, boundingSites(), clip);
            } catch (error) {
                console.log("Cannot compute Convex Hull")
                throw new d3WeightedVoronoiError('Cannot compute Convex Hull');
            }
